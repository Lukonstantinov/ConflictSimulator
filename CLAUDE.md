...
## Changelog
- **3.5.2** — Fix tactical blank screen on mobile + topographic map rendering: fix flex height chain collapse (tactical wrapper uses h-[100dvh] with overflow-hidden, Suspense wraps TacticalView in flex-1 min-h-0 container), PixiJS canvas auto-resizes to fit container via ResizeObserver (auto-fit zoom centers map in viewport, respects user pinch-zoom), topographic/satellite-style terrain rendering (HSL noise-varied colors per tile via SimplexNoise, terrain detail patterns: tree canopy circles, water wave lines, road dashes, trench hatching, rubble debris, grass strokes), elevation shading overlays, contour lines between different elevations, terrain cached as RenderTexture for performance
- **3.5.1** — Fix mobile/iPhone crashes and tactical scenario switching: stop engine before switching scenarios/factions (prevents race conditions and stale unit references), fix PixiJS renderer cleanup on scenario switch (use map identity instead of map.id, preserve canvas element across renderer instances), responsive mobile layout (cap map height to 55% viewport on mobile, hide secondary toolbar buttons on small screens, responsive scenario dropdown width, compact CountryPanel with 40dvh max height, responsive StatsOverlay sizing, EventLog height capped to 25dvh), tactical view minHeight uses min(500px, 60dvh) for mobile fit, improved mobile CSS (touch scrolling, min-height instead of fixed height)
